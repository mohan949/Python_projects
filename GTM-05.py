...
def send_detailed_slack_report(results, csv_file, env_name, webhook_url):
    """
    Sends a detailed report to Slack with all tested pages' results.
    """
    header = (
    ...
    msg = "\n".join(lines)
    resp = requests.post(webhook_url, json={"text": msg})
...
for url, page_name in urls_to_test:
    test_date = datetime.now().strftime("%Y-%m-%d")  # Changed to date only, no timestamp
    ...
    try:
        response = requests.post(BASE_URL + "tests", headers=headers, json=payload, auth=(API_KEY, ""))
    except Exception as e:
        print(f"Error: Failed to send request to GTmetrix API - {e}")
        results.append([test_date, url, env_name, page_name, "", "", "", "", "", "", f"Exception: {e}"])
        continue
    ...
    if response.status_code != 202:
        ...
        results.append([test_date, url, env_name, page_name, "", "", "", "", "", "", f"Failed to start: {err_msg}"])
        continue
    ...
    if not test_id:
        ...
        results.append([test_date, url, env_name, page_name, "", "", "", "", "", "", "No test ID"])
        continue
    ...
    while poll_count < max_polls:
        ...
        try:
            ...
            results.append([test_date, url, env_name, page_name, "", "", "", "", "", "", f"Polling exception: {e}"])
            break
        ...
        if status_resp.status_code == 200:
            ...
            if state == "completed":
                ...
                if report_id:
                    ...
                    results.append([test_date, url, env_name, page_name, "", "", "", "", "", "", "No report ID"])
                    break
            elif state == "error":
                error_msg = attributes.get("error", "Unknown error")
                print(f"Test failed (state=error): {error_msg}")
                results.append([test_date, url, env_name, page_name, "", "", "", "", "", "", f"Error: {error_msg}"])
                break
            else:
                time.sleep(3)
                continue
            ...
            if status_resp.status_code == 404:
                ...
                results.append([test_date, url, env_name, page_name, "", "", "", "", "", "", "Test not found (404)"])
            elif status_resp.status_code == 429:
                print("Error: Rate limit exceeded or too many concurrent requests (429).")
                results.append([test_date, url, env_name, page_name, "", "", "", "", "", "", "Rate limit/concurrency error (429)"])
            else:
                print(f"Unexpected response: HTTP {status_resp.status_code} - {status_resp.text}")
                results.append([test_date, url, env_name, page_name, "", "", "", "", "", "", f"Unexpected response: {status_resp.status_code}"])
            break
    else:
        print("Error: Test did not complete within expected time.")
        results.append([test_date, url, env_name, page_name, "", "", "", "", "", "", "Timeout"])
        continue

    if not report_url:
        print("Error: Report URL could not be determined.")
        results.append([test_date, url, env_name, page_name, "", "", "", "", "", "", "No report URL"])
        continue
    ...
    try:
        ...
        results.append([test_date, url, env_name, page_name, "", "", "", "", "", "", f"Report exception: {e}"])
        continue

    if report_resp.status_code != 200:
        print(f"Failed to get report: HTTP {report_resp.status_code} - {report_resp.text}")
        results.append([test_date, url, env_name, page_name, "", "", "", "", "", "", f"Failed to get report: {report_resp.status_code}"])
        continue
...
# Save all results to CSV - simplified format
csv_file = "gtmetrix_report.csv"
try:
    with open(csv_file, mode="w", newline="") as f:
        writer = csv.writer(f)
        # Updated CSV headers to include the 5 required columns
        writer.writerow(["Date", "Page Name", "Report URL", "Performance Score", "Environment"])
        
        # Write only the required columns for each result
        for row in results:
            test_date, url, env_name, page_name, perf_score, onload_time, fully_loaded, page_bytes, page_requests, report_url_val, error = row
            writer.writerow([test_date, page_name, report_url_val or "-", perf_score or "-", env_name])
    print(f"\nResults saved to {csv_file}")
except Exception as e:
    print(f"Error writing CSV file: {e}")

# --- SLACK WEBHOOK ---
SLACK_WEBHOOK_URL = "https://hooks.slack.com/services/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"  # <-- replace with your real Slack webhook
SLACK_WEBHOOK_URL2 = "https://hooks.slack.com/services/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"  # <-- replace with your real Slack webhook

send_detailed_slack_report(results, csv_file, env_name, SLACK_WEBHOOK_URL)
#send_detailed_slack_report(results, csv_file, env_name, SLACK_WEBHOOK_URL2)
